...
export function PaymentRefundDialog({ payment, open, onOpenChange, onRefundSuccess }: PaymentRefundDialogProps) {
  ...
  const isPartialRefund = refundAmount && parseFloat(refundAmount) < maxRefundAmount
  const isFullRefund = !refundAmount || parseFloat(refundAmount) === maxRefundAmount
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-lg">
            ...
            <Badge variant="outline" className="ml-auto">
              {refundAmount ? formatCurrency(parseFloat(refundAmount)) : formatCurrency(maxRefundAmount)}
            </Badge>
          </div>
          ...
          <div className="flex items-start gap-3 p-3 bg-yellow-50 border-l-4 border-yellow-400 rounded-r-lg">
            ...
            <div className="text-sm text-yellow-800">
              ...
              <ul className="mt-1 space-y-1 text-xs">
                <li>• Hoàn tiền sẽ được xử lý qua cùng phương thức thanh toán gốc</li>
                <li>• Thời gian xử lý có thể từ 3-7 ngày làm việc</li>
                <li>• Thao tác này không thể hoàn tác</li>
                {isPartialRefund && <li>• Đây là hoàn tiền một phần, số tiền còn lại vẫn được giữ</li>}
              </ul>
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={loading || (refundAmount !== "" && (parseFloat(refundAmount) <= 0 || parseFloat(refundAmount) > maxRefundAmount))}
            className="bg-red-600 hover:bg-red-700"
          >
            {loading ? "Đang xử lý..." : `Hoàn tiền ${refundAmount ? formatCurrency(parseFloat(refundAmount)) : formatCurrency(maxRefundAmount)}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
